...
## Define the Job Submission Frame
# Define the user input region for job name
...
############################ This block is not available for now ##################################
## Make a automatic download function when the job is done
# enable_ad = ["yes", "no"]
...
# Define the job settings for submission
def submit_job():
    ...
    JOBcalculix.appenddata_job(csvfile, job_id, jobname, submit_date)
...
def download_log():
    ...
    FILEcalculix.download_poutlog(save_path, job_id)
...
def download_job():
    ...
    FILEcalculix.download_outputfile(apibaseurl, token, job_id, jobname, save_path)
